...
class auto_patrol_node(BasicNavigator):
    def __init__ (self,node_name='patrol_node'):
        ...
        self.declare_parameter('image_save_path','/home/ros2_humble_env/task3_ws/src/auto_partol_package/image/')
        ...
        self.sub_image=self.create_subscription(Image,'/camera_sensor/image_raw',self.image_callback,1)

    def image_callback(self,msg):
        self.laster_image=msg

    def save_image(self):
        if self.laster_image is not None:
            pose = self.get_current_pose()
            cv_image = self.cv_bridge_.imgmsg_to_cv2(self.laster_image)
            result=self.model.predict(cv_image)
            result_image=result[0].plot()
            cv2.imwrite(
                f'{self.image_save_path_}image_{pose.translation.x:3.2f}_{pose.translation.y:3.2f}.png',
                result_image
            )
    ...
    def nav_to_goal(self,goal_point): 
        ...
        if goal_point.pose.orientation.w==0.0:
    ...
    for idx,action_dict in enumerate(patrol_node.nav2_action_):
      for action_key,action_value in action_dict.items():
        if action_key=="goal_points":
            ...
            x_past=x
            y_past=y
            patrol_node.save_image()
            patrol_node.get_logger().info(f"到达目标点，保存图像完成")
        elif action_key =="yaw":
            yaw=action_value
